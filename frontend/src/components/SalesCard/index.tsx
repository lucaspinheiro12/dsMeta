...
function SalesCard() {
    ...
    useEffect( () => {
        ...
        const data_min = minDate.toISOString().slice(0, 10);
        const data_max = maxDate.toISOString().slice(0, 10);

        axios.get(`${BASE_URL}/sales?minDate=${data_min}&maxDate=${data_max}`).then(response => {
